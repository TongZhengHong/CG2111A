...
void manualMove(float speed, float dist, long ang, char dir)
{
  int val = pwmVal(speed);
  unsigned long temp;
  ...
  switch (dir) {
    ...
    case 'a':
      left();
      deltaTicks = (ang / 360.0) * (ALEX_CIRC / WHEEL_CIRC) * COUNTS_PER_REV;
      targetTurnTicks = leftReverseTicks + deltaTicks;
      while (leftReverseTicks <  targetTurnTicks);
      stop();
      break;
    case 's':
      reverse();
    ...
    case 'd':
      ...
      deltaTicks = (ang / 360.0) * (ALEX_CIRC / WHEEL_CIRC) * COUNTS_PER_REV;
      targetTurnTicks = rightReverseTicks + deltaTicks;
      while (rightReverseTicks < targetTurnTicks);
